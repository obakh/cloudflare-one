...
const CustomListItem = ({
	...
	<li className={cn("py-0 my-0 leading-relaxed", className)} {...props} data-streamdown="list-item">
		{children}
	</li>
);
		...
		<Streamdown
			...
			components={{
				...
				h2: ({ children, node, ...props }) => (
					<h3 className="font-medium text-sm text-primary tracking-wide" {...props}>
						{children}
					</h3>
				),
				...
				h4: ({ children, node, ...props }) => (
					<h4 className="font-medium text-sm text-primary tracking-wide" {...props}>
						{children}
					</h4>
				),
				p: ({ children, ...props }) => (
				...
				a: (props) => {
					return <a {...props} className="underline" target="_blank" rel="noopener noreferrer" />;
				},
			}}
			{...props}
		/>
